                    ...
                    const cctag::Parameters* params)
{
    ...
    if(imgGraySrc.type() != CV_8UC1 || imgCanny.type() != CV_8UC1)
    ...
    if(params->_debugDir == "")
    {
        std::cerr << __FUNCTION__ << ":" << __LINE__ << ": debugDir not set, not writing debug output" << std::endl;
    }
    else
    {
        std::cerr << __FUNCTION__ << ":" << __LINE__ << ": debugDir is [" << params->_debugDir
                  << "] using that directory" << std::endl;
        ...
        *mag_img_file << "P5" << std::endl << size.width << " " << size.height << std::endl << "255" << std::endl;
        ...
        *hyst_img_file << "P5" << std::endl << size.width << " " << size.height << std::endl << "255" << std::endl;
    }
...
#endif
                    CCTAG_IF_DEBUG(if(_imgDX[j] > 200) { CCTAG_COUT_NOENDL(_imgDX[j] << ", " << _imgDY[j] << ", "); })
                }
            }
        }
        else
